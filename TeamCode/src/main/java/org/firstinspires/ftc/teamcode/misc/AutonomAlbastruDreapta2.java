...
@Autonomous(name="AutonomAlbastruDreapta2")
@Disabled
public class AutonomAlbastruDreapta2 extends LinearOpMode{
    ...
    public void runOpMode() {
        ...
        StopAllMotion();
        DriveForward(40, 0.5, 0.5, 0.5, 0.5);
        ...
        if(robot.distanta.getDistance(DistanceUnit.CM) <30) {
            ...
            sleep(80);
            StopAllMotion();
            DriveForward(33, 0.5, 0.5, 0.5, 0.5);
            StopAllMotion();
            sleep(500);
            ...
            sleep(3);
            DriveBackward(33, 0.6, 0.6, 0.6, 0.6);
            StopAllMotion();
            robot.ridicare.setPower(0);
            StopAllMotion();
            RotateLeft(82);
            StopAllMotion();
            DriveBackward(175, 0.5, 0.5, 0.5, 0.5);
            StopAllMotion();
            StrafeRight(40, 0.5);
            sleep(1000);
        ...
        else
        {
            ...
            if (robot.distanta.getDistance(DistanceUnit.CM) <= 35)
            {
                ...
                robot.ridicare.setPower(-0.1);
                StopAllMotion();
                DriveBackward(60, 0.5, 0.5, 0.5, 0.5);
                StopAllMotion();
                RotateLeft(90);
                StrafeRight(80, 0.4);
                ///StrafeLeft(5, 0.4);
                StopAllMotion();
                DriveForward(160, 0.5, 0.5, 0.5, 0.5);
                StrafeLeft(110 , 0.4);
            }
            else
            if (robot.distanta.getDistance(DistanceUnit.CM) > 35)
    ...
    public void EncoderStrafeBrat(double speed, double distance, double timeoutS, int ticks, double putereBrat) {
        int newBackLeftTarget;
        int newBackRightTarget;
        int newFrontLeftTarget;
        int newFrontRightTarget;
        if (opModeIsActive()) {
            ...
            robot.ridicare.setMode(DcMotor.RunMode.RUN_USING_ENCODERS);
            robot.ridicare.setTargetPosition(ticks);
            ...
            robot.LeftFrontMotor.setPower(speed);
            robot.RightFrontMotor.setPower(speed);
            robot.ridicare.setPower(putereBrat);
            while (opModeIsActive() &&
                    (runtime.seconds() < timeoutS) &&
                    (
                            robot.LeftBackMotor.isBusy() && robot.RightBackMotor.isBusy() &&
                                    robot.LeftFrontMotor.isBusy() && robot.RightFrontMotor.isBusy() && robot.ridicare.isBusy()
                    )
                    )
            {
